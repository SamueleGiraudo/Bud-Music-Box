 ...
 * Creation: mar. 2019
 * Modifications: mar. 2019, aug. 2019, sep. 2019
 *)

(* A multi-pattern is a nonempty list of patterns. *)
type multi_pattern = Pattern.pattern list

(* Tests if the list lst of patterns is a multi-pattern. A list of
 * patterns is a multi-pattern if it is not empty and all its patterns
 * have the same length and the same arity. *)
let is_multi_pattern lst =
    if lst = [] then
        ...
        lst |> List.for_all
            (fun p -> Pattern.length p = len && Pattern.arity p = ar)
...
(* Returns the multi-pattern encoded by the string str. For instance,
 * "* 1 * 2 ; * * 0 -1" is a 2-multi-pattern. *)
let from_string str =
    Tools.list_from_string Pattern.from_string ';' str

(* Returns the k-multi-pattern consisting in k voices of the empty
 * pattern. *)
let empty k =
    assert (k >= 1);
    Tools.interval 1 k |> List.map (fun _ -> Pattern.empty)

(* Returns the k-multi-pattern consisting in k voices of the unit
 * pattern. *)
let unit k =
    assert (k >= 1);
...
(* Returns the i-th pattern of the multi-pattern mpat. The indexation
 * begins at 1. *)
let pattern mpat i =
    assert (is_multi_pattern mpat);
...
(* Returns the multi-pattern obtained by the partial composition of the
 * multi-patterns mpat_1 and mpat_2 at position i. *)
let partial_composition mpat_1 i mpat_2 =
    ...
    pairs |> List.map
        (fun (seq_1, seq_2) ->
            Pattern.partial_composition seq_1 i seq_2)
