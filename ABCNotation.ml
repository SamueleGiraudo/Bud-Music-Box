...
(* Returns a string representing the pattern pat under the context
 * context in the abc notation. *)
let pattern_to_abc_string context pat =
    ...
    let str = deg_dur |> List.map
        (fun (deg, dur) ->
            let note = midi_to_abc_note
                (Context.degree_to_midi_note context deg) in
            (Printf.sprintf "%s%d" note dur)) in
    first_rest_str ^ (String.concat " " str)

(* Returns a string representing the multi-pattern mpat under the
 * context context in the abc notation. *)
...
(* Returns a string representing the multi-pattern mpat under the
 * context context in the abc notation. This string contains all the
 * information to be a valid abc program. *)
let complete_abc_string context mpat =
    assert (MultiPattern.is_multi_pattern mpat);
    let res = "" in
    let res = res ^ "X:1\n" in
    let res = res ^ "T:Music\n" in
    let res = res ^ "C:By music box operad\n" in
    let res = res ^ "K:Am\n"in
    let res = res ^ "M:8/8\n" in
    let res = res ^ "L:1/8\n" in
