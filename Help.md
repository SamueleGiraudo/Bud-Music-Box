...
+ **Integers** are expressed in the usual signed decimal notation.

  For instance, `0`, `127`, `-0`, `-17`,  and `- 17` are integers,

+ **Identifiers** are strings of length at least $1$ and made of symbols in `a`-`z`,
  `A`-`Z`, `0`-`9`, `'`, or `_`, and starting with an alphabetic symbol or `'` or `_`.
...
+ `NAME` is an identifier of a multi-pattern, bounded to a multi-pattern.
...
Possible errors:
+ `?? Unknown multi-pattern name.`. Occurs when `NAME` is not bounded.
