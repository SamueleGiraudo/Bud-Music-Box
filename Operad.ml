...
let create arity partial_composition unit =
    {arity = arity;
...
(* Returns the full composition map of the operad op. *)
let full_composition op x lst =
    ...
    indexed_elements |> List.fold_left
        (fun res (i, y) -> op.partial_composition res i y) x

(* Returns the binary composition map of the operad op. *)
