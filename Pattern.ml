 ...
 * Creation: mar. 2019
 * Modifications: mar. 2019, apr. 2019, aug. 2019, sep. 2019
 *)

(* A pattern is a list of atoms. There is no condition: all lists are
 * valid patterns, even the empty list. *)
 ...
 * For instance, "* 1 * * * 2 -1 8 * 12 0" encodes a pattern, where the
 * "*" are rests. Each beat or rest must be separated by at least one
 * space. *)
let from_string str =
...
(* Returns the pattern consisting in one atom of degree deg followed by
 * duration - 1 rests. This is in fact a degree with duration as
 * duration. *)
let beat deg duration =
    assert (duration >= 1);
    let rests = (Tools.interval 2 duration) |> List.map
        (fun _ -> Atom.Rest) in
    (Atom.Beat deg) :: rests
...
(* Returns the pattern obtained by concatenating the patterns of the
 * list of patterns pattern_lst. *)
let concat pattern_lst =
    List.concat pattern_lst

(* Returns the arity of the pattern pat. This is the number of beats of
 * the pattern. *)
let arity pat =
    pat |> List.fold_left
        (fun res a -> if Atom.is_beat a then 1 + res else res) 0

(* Returns the length of the pattern pat. This is the number of atom of
 * the pattern. *)
let length pat =
    List.length pat
...
let extract_durations pat =
    let rec aux lst =
        match lst with
            ...
            |(Atom.Beat d) :: lst' ->
                (1 + (Tools.length_start lst' Rest)) :: aux lst'
    in
    aux pat

(* Returns the pattern obtained by the partial composition of the
 * patterns pat_1 and pat_2 at position i. *)
